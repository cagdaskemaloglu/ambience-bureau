...
import { htmlToPdfBuffer, mergeHtmlPagesToPdf, withBrowser } from './generatePdf'
  ...
  await withBrowser(async (browser) => {
    for (const item of order.order_items ?? []) {
      try {
        ...
        const certificatePdf = await htmlToPdfBuffer(browser, certificateHtml)
        ...
        const productCardPdf = await mergeHtmlPagesToPdf(browser, [frontHtml, backHtml])
        ...
        const warrantyPdf = await htmlToPdfBuffer(browser, warrantyHtml)
